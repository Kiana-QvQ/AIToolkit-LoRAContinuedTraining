...
export interface AddBaseLoRAModalState {
  onComplete?: (loraPath: string | null) => void;
}

export const addBaseLoRAModalState = createGlobalState<AddBaseLoRAModalState | null>(null);

export const openAddBaseLoRAModal = (onComplete: (loraPath: string | null) => void) => {
  addBaseLoRAModalState.set({ onComplete });
};

export default function AddBaseLoRAModal() {
  const [modalInfo, setModalInfo] = addBaseLoRAModalState.use();
  ...
  const onDone = (loraPath: string | null) => {
    if (modalInfo?.onComplete && !isUploading) {
      modalInfo.onComplete(loraPath);
      setModalInfo(null);
    }
  };
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      const resp = await apiClient.post('/api/loras/upload', formData, {
        headers: {
            ...
            <div className="bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div className="text-center">
                ...
                <div className="text-xs text-gray-400 mb-3">
                  Choose a local `.safetensors` file. It will be uploaded into the AI Toolkit LoRA directory and used by
                  the training job.
                </div>
                <div className="w-full">
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-200 text-center">
                      {isDragActive ? 'Drop the LoRA file here...' : 'Drag & drop a `.safetensors` file here, or click to select one'}
                    </p>
                  </div>
                  {isUploading && (
                    <div className="mt-4">
